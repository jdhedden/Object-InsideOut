...
This document describes Object::InsideOut version 3.89
...
using code in the C<:Default> directive.
 ...
 my @bar :Field  :Default(rand 100);
 my @foo :Field  :Default(our $next; ++$next);

The above, for example, will initialize the C<bar> attribute of each new
object to a different random number, and C<foo> will be a sequential integer.
...
Note that (due to the limitations of the Perl variable attribute mechanism)
any code specified as a default will I<not> have access to the surrounding
lexical scope, nor can you use parens in that code.  For example, this will
not work (nor even compile):
...
For anything complex or lexical, you should factor the initializer out into
...
that method delegate to your I<Brakes> object by calling its C<engage()> method
instead.
